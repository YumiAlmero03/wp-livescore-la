<?php
/**
 * Server render for Team Filters block.
 *
...
$title        = isset( $attributes['title'] ) ? sanitize_text_field( $attributes['title'] ) : __( 'Filter Teams', 'wp-livescore-la' );
...
$selected_sport   = isset( $_GET['team_sport'] ) ? sanitize_title( wp_unslash( $_GET['team_sport'] ) ) : '';
$selected_league  = isset( $_GET['team_league'] ) ? sanitize_title( wp_unslash( $_GET['team_league'] ) ) : '';
$selected_country = isset( $_GET['team_country'] ) ? sanitize_title( wp_unslash( $_GET['team_country'] ) ) : '';
$team_archive     = get_post_type_archive_link( 'team' );

if ( ! $team_archive ) {
	$team_archive = home_url( '/teams/' );
}
...
$wrapper_attributes = get_block_wrapper_attributes(
	array(
		'class' => 'wp-livescore-la-team-filters',
	)
);
?>
<section <?php echo wp_kses_data( $wrapper_attributes ); ?>>
	<?php if ( $show_title && '' !== $title ) : ?>
		<h2 class="wp-livescore-la-team-filters__title"><?php echo esc_html( $title ); ?></h2>
	<?php endif; ?>

	<form class="wp-livescore-la-team-filters__form" method="get" action="<?php echo esc_url( $team_archive ); ?>">
		<?php if ( $show_sport ) : ?>
			<label class="wp-livescore-la-team-filters__field">
				<span><?php esc_html_e( 'Sport', 'wp-livescore-la' ); ?></span>
				<select name="team_sport">
			...
			<label class="wp-livescore-la-team-filters__field">
				...
				<select name="team_league">
					...
					<?php foreach ( $leagues as $league ) : ?>
						...
						<option value="<?php echo esc_attr( $league_slug ); ?>" <?php selected( $selected_league, $league_slug ); ?>>
		...
		<div class="wp-livescore-la-team-filters__actions">
			<button type="submit" class="wp-livescore-la-team-filters__submit"><?php echo esc_html( $button_text ); ?></button>
			<a class="wp-livescore-la-team-filters__reset" href="<?php echo esc_url( $team_archive ); ?>"><?php echo esc_html( $reset_text ); ?></a>
		</div>
	</form>
</section>
